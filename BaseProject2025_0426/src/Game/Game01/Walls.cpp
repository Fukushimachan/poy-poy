...
bool Walls::Init()
{
    ...
    for(int a = 0; a < 4; ++a) {
        for(int i = 0; i < 4; ++i) {
            auto wall = Scene::Object::Create<Wall>()->SetName("Wall");
...
void Walls::Update()
{
    ...
    auto wall_coll = wall->GetComponent<ComponentCollisionCapsule>();
    for(int a = 0; a < 4; ++a) {
        for(int i = 0; i < 4; ++i) {
            //auto wall = Scene::Object::Create<Wall>()->SetName("Wall");
...
void Walls::OnHit(const ComponentCollision::HitInfo &hit_info)
{
}
}    // namespace Game01
